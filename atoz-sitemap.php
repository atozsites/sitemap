<?php
/**
Plugin Name: AtoZ Sitemap
Plugin URI: https://github.com/atozsites
Description: Add a sitemap on any page/post using the simple shortcode [AtoZ_sitemap]
Version: 1.5.6
Author: AtoZ team
...
function wsp_load_textdomain() {
  load_plugin_textdomain( 'AtoZ_sitemap', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
}
...
if ( !defined('WSP_PLUGIN_NAME') ) {
	define('WSP_PLUGIN_NAME', 'AtoZ_sitemap');
}
...
function wsp_plugin_row_meta($links, $file) {
	if ($file == plugin_basename(__FILE__)) {
		$settings_page = 'AtoZ_sitemap';
		$links[] = '<a href="options-general.php?page=' . $settings_page .'">' . __('Settings','AtoZ_sitemap') . '</a>';
		$links[] = '<a href="' . WSP_DONATE_LINK . '">'.__('Donate', 'AtoZ_sitemap').'</a>';
	}
	return $links;
}
...
function wsp_show_tabs() {
	...
	// All tabs
	$tabs = array();
	$tabs['main']    = __('Settings', 'AtoZ_sitemap');
	$tabs['about']   = __('How to use', 'AtoZ_sitemap');
 ...
 * Use like this : [AtoZ_sitemap]
 * 
 * @param $atts
 * @param $content
 * @return str $return
 */
function wsp_AtoZ_sitemap_func( $atts, $content=null ) {
	...
	if ($wsp_is_display_copyright==1) {
		$copyright_link = '<p><a href="http://wordpress.org/plugins/wp-sitemap-page/">'.__('Powered by "AtoZ Sitemap Page"', 'AtoZ_sitemap').'</a></p>';
	}
...
add_shortcode( 'AtoZ_sitemap', 'wsp_AtoZ_sitemap_func' );
...
function wsp_return_content_type_page($is_title_displayed=true, $is_get_only_private=false, $display_nofollow=false, $wsp_exclude_pages, $sort=null) {
	...
	if ($is_title_displayed==true) {
		$return .= '<h2 class="wsp-pages-title">'.__('Pages', 'AtoZ_sitemap').'</h2>'."\n";
	}
...
function wsp_return_content_type_post( $is_title_displayed=true, $display_nofollow=false, $display_post_only_once, $wsp_exclude_pages=array(), $sort_categories=null ) {
	...
	// add content
	if ($is_title_displayed==true) {
		$return .= '<h2 class="wsp-posts-title">'.__('Posts by category', 'AtoZ_sitemap').'</h2>'."\n";
	}
...
function wsp_return_content_type_categories( $is_title_displayed=true, $display_nofollow=false, $sort=null ) {
	...
	// add content
	if ($is_title_displayed==true) {
		$return .= '<h2 class="wsp-categories-title">'.__('Categories', 'AtoZ_sitemap').'</h2>'."\n";
	}
...
function wsp_return_content_type_tag($is_title_displayed=true, $display_nofollow=false) {
	...
	// add content
	if ($is_title_displayed==true) {
		$return .= '<h2 class="wsp-tags-title">'.__('Tags', 'AtoZ_sitemap').'</h2>'."\n";
	}
...
function wsp_return_content_type_archive($is_title_displayed=true, $display_nofollow=false) {
	...
	// add content
	if ($is_title_displayed==true) {
		$return .= '<h2 class="wsp-archives-title">'.__('Archives', 'AtoZ_sitemap').'</h2>'."\n";
	}
...
function wsp_return_content_type_author( $is_title_displayed=true, $display_nofollow=false, $sort=null ) {
	...
	// add content
	if ($is_title_displayed==true) {
		$return .= '<h2 class="wsp-authors-title">'.__('Authors', 'AtoZ_sitemap').'</h2>'."\n";
	}
...
function wsp_htmlFromMultiArray( array $nav = array() , $useUL = true, $display_post_only_once = true, $display_nofollow=false, $wsp_exclude_pages = array(), $sort=null ) {
	...
	foreach ($nav as $page) {
		$html .= "\t".'<li><strong class="wsp-category-title">'
			.sprintf( __('Category: %1$s', 'AtoZ_sitemap'), '<a href="'.get_category_link($page->cat_ID).'"'.$attr_nofollow.'>'.$page->name.'</a>' )
			.'</strong>'."\n";
...
function wsp_displayPostByCat( $cat_id, $display_post_only_once=true, $display_nofollow=false, $wsp_exclude_pages=array(), $sort=null ) {
	...
	if ( $wsp_posts_by_category === false ) {
		...
		$wsp_posts_by_category = sprintf(__('<a href="{permalink}"%1$s>{title}</a> ({monthnum}/{day}/{year})', 'AtoZ_sitemap'), $attr_nofollow);
